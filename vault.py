...
read_devgrid_response = client.secrets.kv.v2.read_secret_version(
            path='devgrid', mount_point='secret')
print(f"Here is the secret for devgrid db name is : {read_devgrid_response['data']['data']['db_name']}")
print(f"Here is the secret for devgrid password is: {read_devgrid_response['data']['data']['password']}")

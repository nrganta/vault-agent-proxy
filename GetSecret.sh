...
vault kv get -namespace=admin secret/devgrid
vault kv get -namespace=admin secret/lngrid
...
curl \
	    --header "X-Vault-Token: $VAULT_TOKEN" \
	        $VAULT_AGENT_ADDR/v1/admin/secret/data/devgrid | jq
curl \
	            --header "X-Vault-Token: $VAULT_TOKEN" \
		                    $VAULT_AGENT_ADDR/v1/admin/secret/data/lngrid | jq
